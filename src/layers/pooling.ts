...
export function pool3d(
    x: Tensor, poolSize: [number, number, number],
  ...
  return tidy(() => {
    ...
    x = preprocessConv3DInput(x, dataFormat);  // x is NHWC after preprocessing.
    let y: Tensor;
    const paddingString = (padding === 'same') ? 'same' : 'valid';
    if (poolMode === 'max') {
      // TODO(cais): Rank check?
      y = tfc.maxPool3D(
          x as tfc.Tensor<tfc.Rank.R5>, poolSize, strides, paddingString);
    } else {  // 'avg'
      throw new NotImplementedError(
          '3D average pooling is not implemented yet.')
    }
    if (dataFormat === 'channelsFirst') {
      y = tfc.transpose(y, [0, 4, 1, 2, 3]);  // NHWC -> NCHW.
    }
    return y;
  });
}
...
export declare interface Pooling3DLayerArgs extends LayerArgs {
  /**
   * Factors by which to downscale in each dimension [depthwise, vertical,
   * horizontal]. Expects an integer or an array of 3 integers.
   *
   * For example, `[2, 2, 2]` will halve the input in all three spatial
   * dimensions. If only one integer is specified, the same window length will
...
export abstract class Pooling3D extends Layer {
  ...
  computeOutputShape(inputShape: Shape|Shape[]): Shape|Shape[] {
    ...
    dept =
        convOutputLength(dept, this.poolSize[0], this.padding, this.strides[0]);
    rows =
    ...
    if (this.dataFormat === 'channelsFirst') {
      return [inputShape[0], inputShape[1], dept, rows, cols];
    } else {
      return [inputShape[0], dept, rows, cols, inputShape[4]];
    }
  }
...
/**
 * Max pooling operation for volumetric data.
 *
 * Input shape
 *   - If `dataFormat === CHANNEL_LAST`:
 *       5D tensor with shape:
 *       `[batchSize, depth, rows, cols, channels]`
 *   - If `dataFormat === CHANNEL_FIRST`:
 *      5D tensor with shape:
 *       `[batchSize, channels, depth, rows, cols]`
 *
 * Output shape
 *   - If `dataFormat=CHANNEL_LAST`:
 *       5D tensor with shape:
 *       `[batchSize, poolDepth, pooleRows, pooledCols, channels]`
 *   - If `dataFormat=CHANNEL_FIRST`:
 *       5D tensor with shape:
 *       `[batchSize, channels, poolDepth, pooleRows, pooledCols]`
 */
